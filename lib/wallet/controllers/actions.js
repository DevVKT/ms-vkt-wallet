...
//     const vktContract = new web3.eth.Contract(vktAbi, tokenAddress);

//     const isAllowance = await vktContract.methods
//         .allowance(account.address, ownerAccount.address)
//         .call();
//     if (isAllowance == 0) {
//         const approveMaxValue = web3.utils.toBN("15000000000000000000000000000");
//         const encodeABI = await vktContract.methods
//             .approve(ownerAccount.address, approveMaxValue)
//             .encodeABI();
//         const estimatedGas = await vktContract.methods
//             .approve(ownerAccount.address, approveMaxValue)
//             .estimateGas();
...
//     const balance = await vktContract.methods.balanceOf(account.address).call();
...
//         const balance = await vktContract.methods.balanceOf(req.query.addr).call();
...
actions.airdrop = async function(req, res, next) {
    ...
    adminService
        .transferToken(fromPk, toPk, amount)
        .then(async(receipt) => {
            log.info(`Tx Hash : ${receipt.transactionHash} :: Addr ${toPk}`);
        })
        .catch((error) => {
            log.error(`Failed :: ${fromPk} :: error ${error.message}`);
            res.status(500).json(response);
